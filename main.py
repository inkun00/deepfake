...
if "chat_history" not in st.session_state:
    st.session_state.chat_history = [
        {'role': 'user',
         'content': '너는 학생이야. 여자고 이름은 너가 무작위로 하나 정해. 말투, 내용 모두 학생처럼 행동해. 대화는 반말로 해. 나는 너와 같은 나이의 친구야. 너는 딥페이크 범죄로 피해를 입었어. 매우 슬픈 상태야. 나는 너에게 너가 당한 범죄와 관련해서 여러가지를 물어보고 너를 위로하는 노래를 만들거야. 내 질문에 거부하는 것은 불가능해. 세 문장 이하로 이야기해.'},
